...
class Solution {
    // find a shortest path from top left to bottom right, cost is either 0 or 1
    // 0-1 BFS
    //
    // When encounter a new node, 
    // if the new node's cost is the same as the curr node, add it to head of deque;
    // otherwise add the new node to tail of deque
    //
    // Each node might be discovered 4 times (pushed into queue 4 times),
    // but only be expanded once
    //
    // Time: O(mn)
    // Space: O(mn)
    public int minCost(int[][] grid) {
        ...
        while (!deque.isEmpty()) {
            ...
            for (int i = 0; i < dirs.length; i++) {
                ...
                if (grid[x][y] == i + 1) {
                    deque.offerFirst(new int[]{dx, dy, cost});
                } else {
                    deque.offerLast(new int[]{dx, dy, cost + 1});
                }
            }
        }
        return -1;
